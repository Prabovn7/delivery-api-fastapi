...
async def criar_conta(usuario_schema: UsuarioSchema, session: Session = Depends(pegar_sessao), admin: Usuario = Depends(verificar_token)):
    ...
    if usuario:
        ...
        if usuario_schema.admin == True:
            if not admin.admin:
                raise HTTPException(status_code=401,detail="Você não é administrador para criar outro usuario admin")
